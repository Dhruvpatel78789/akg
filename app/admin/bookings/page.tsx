...
export default function AdminBookingsPage() {
  ...
  return (
    <section className="min-w-0 pb-10">
      <h1 className="text-4xl font-black text-[var(--primary)]">Bookings</h1>
                    ...
                    {visibleColumns.payment && (
                      <td className="text-xs">
                        {booking.coinCost && booking.coinCost > 0 ? (
                          ...
                          <>
                            <p className="font-semibold text-gray-700">₹{booking.price || 0} ({booking.paymentMode || "online"})</p>
                            {booking.transactionId && <p className="text-[10px] text-gray-400">Txn: {booking.transactionId}</p>}
                          </>
                        )}
                        <p className="text-[10px] font-bold text-gray-500 uppercase">{booking.paymentStatus || "PAID"}</p>
                      </td>
                    )}
      ...
      {selectedDetailsBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 overflow-y-auto">
          <div className="w-full max-w-lg bg-white rounded-3xl p-6 shadow-xl relative animate-fade-in space-y-4 my-8">
            <button
