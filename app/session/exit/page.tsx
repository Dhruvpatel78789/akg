...
import { LogIn, Building2, User, CheckCircle2, AlertCircle, Loader2, ArrowRight } from "lucide-react";

function ExitHandlerForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/player/qr-exit?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
  ...
  async function handleCheckout(e: React.FormEvent) {
    e.preventDefault();
    if (selectedSessions.length === 0 || submitting) return;
  ...
  if (!user) {
    const redirectUrl = `/session/exit?token=${encodeURIComponent(token)}`;
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-rose-50 border border-rose-200 p-4 rounded-2xl text-center text-rose-800 font-bold flex items-center gap-2">
      ...
      {!success && (
        <form onSubmit={handleCheckout} className="space-y-4">
          ...
          {activeSessions.length > 0 && (
            ...
            >
              {submitting ? "Ending Sessions..." : "Complete Exit & Checkout"}
            </button>
          )}
        </form>
      )}
...
export default function ExitHandlerPage() {
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-12 flex items-center justify-center">
      <section className="w-full max-w-md bg-white p-8 rounded-3xl shadow-xl ring-1 ring-black/5">
        <Suspense fallback={
          <div className="flex flex-col items-center justify-center p-12 text-center">
            <Loader2 className="animate-spin text-[var(--primary)] mb-3" size={32} />
            <p className="font-bold text-[var(--primary)]">Loading exit scanner...</p>
          </div>
        }>
          <ExitHandlerForm />
        </Suspense>
